...
The effect of BMI of the relative amounts of nondiabetics versus diabetics is shown here.
...
knitr::kable(comparison_results, digits = 4, align = "ll")
```
...
#print results
knitr::kable(test_results_logistic, digits = 4)
```
...
#print results
knitr::kable(test_results_lasso, digits = 4)
```
...
#print results
knitr::kable(test_results_ridge, digits = 4)
```
...
```{r,comparison}
comparison_results <-bind_rows(test_results_logistic,
                               test_results_lasso,
...
Based on accuracy in prediction of the test set, the "best model" (of the models studied) for Education = `r params$EducationLevel` is `r comparison_results[1,1]` which has accuracy = 
`r round(comparison_results[1,3],4)`.
  


```{r,comparison Log Loss}
comparison_results2 <-comparison_results %>%
    arrange(LogLoss)

```
If comparisons are made based on Log Loss in prediction of the test set, the "best model" (of the models studied) for Education = `r params$EducationLevel` is `r comparison_results2[1,1]` with Log Loss = `r round(comparison_results2[1,2],4)`.
